...
const categories = [
  ...
  'Beauté & Santé', 'Sports & Loisirs', 'Livres & Médias', 'Autres',
];
...
export default function VendorAddProduct() {
  ...
  const handleImageUpload = useCallback(async (files: FileList | null) => {
    ...
    if (remaining <= 0) { toast.error(`Maximum ${MAX_IMAGES} images atteint`); return; }
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({ type: 'success', text: isEditing ? 'Produit mis à jour' : publish ? 'Produit publié avec succès' : 'Brouillon enregistré' });
  ...
  const discountPercent = compareAtPrice && price && parseFloat(compareAtPrice) > parseFloat(price)
    ? Math.round(((parseFloat(compareAtPrice) - parseFloat(price)) / parseFloat(compareAtPrice)) * 100) : 0;

  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" onClick={() => setCurrentView('vendor-products')} className="shrink-0 cursor-pointer">
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold">{isEditing ? 'Modifier le produit' : 'Ajouter un produit'}</h1>
          <p className="text-sm text-muted-foreground">{isEditing ? 'Modifiez les informations du produit' : 'Remplissez les informations de votre nouveau produit'}</p>
        </div>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-lg flex items-center gap-2"><Package className="h-5 w-5 text-emerald-600" />Informations du produit</CardTitle>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="space-y-2">
            <Label htmlFor="product-name">Nom du produit *</Label>
            <Input id="product-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Smartphone Samsung Galaxy A54" />
          </div>

          <div className="space-y-2">
            <Label htmlFor="product-description">Description</Label>
            <Textarea id="product-description" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Décrivez votre produit en détail..." rows={5} className="min-h-[120px]" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={category} onValueChange={setCategory}>
              <SelectTrigger className="w-full cursor-pointer"><SelectValue placeholder="Sélectionner une catégorie" /></SelectTrigger>
              <SelectContent>
                {categories.map((cat) => <SelectItem key={cat} value={cat} className="cursor-pointer">{cat}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          ...
          <div>
            <button type="button" className="text-sm text-emerald-600 hover:text-emerald-700 cursor-pointer" onClick={() => setShowAdvanced(!showAdvanced)}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={weightUnit} onValueChange={setWeightUnit}>
                    ...
                    <SelectContent>
                      <SelectItem value="g" className="cursor-pointer">Grammes (g)</SelectItem>
                      <SelectItem value="kg" className="cursor-pointer">Kilogrammes (kg)</SelectItem>
                      <SelectItem value="lb" className="cursor-pointer">Livres (lb)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
          ...
          <div className="flex items-center justify-between p-4 rounded-xl bg-muted/30 border">
            <div className="flex items-center gap-3">
              {isPublished ? <Eye className="h-5 w-5 text-emerald-600" /> : <FileEdit className="h-5 w-5 text-amber-600" />}
              <div>
                <p className="font-medium text-sm">{isPublished ? 'Publié' : 'Brouillon'}</p>
                <p className="text-xs text-muted-foreground">{isPublished ? 'Le produit sera visible par les clients' : 'Le produit sera enregistré comme brouillon'}</p>
              </div>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <Label>Images {images.length > 0 && <span className="text-xs text-muted-foreground font-normal">({images.length}/{MAX_IMAGES})</span>}</Label>
            </div>
            <div className="border-2 border-dashed rounded-xl p-6 text-center transition-colors border-muted-foreground/30">
              <input
                type="file"
                accept="image/*"
                multiple
              ...
              <label htmlFor="image-upload" className="cursor-pointer block">
                {uploading ? (
                  ...
                  <>
                    ...
                    <p className="text-sm font-medium text-muted-foreground">Cliquez pour sélectionner des images</p>
                    <p className="text-xs text-muted-foreground mt-1">PNG, JPG — Maximum {MAX_IMAGES} images</p>
                  </>
                )}
              </label>
            </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                {images.map((img, index) => (
                  <div key={index} className="relative aspect-square rounded-lg overflow-hidden border bg-muted group">
                    ...
                    {index === 0 && <Badge className="absolute bottom-1 left-1 text-[9px] bg-emerald-600 text-white border-0 px-1.5 py-0">Principal</Badge>}
                    <button type="button" onClick={() => handleRemoveImage(index)} className="absolute top-1 right-1 h-6 w-6 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md cursor-pointer">
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                ))}
                {images.length < MAX_IMAGES && (
                  <label className="aspect-square rounded-lg border-2 border-dashed border-muted-foreground/30 hover:border-emerald-500 flex flex-col items-center justify-center cursor-pointer transition-colors bg-muted/30">
                    ...
                    <input type="file" accept="image/*" multiple onChange={(e) => handleImageUpload(e.target.files)} className="hidden" disabled={uploading} />
                  </label>
                )}
              </div>
            )}
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-2">
            <button
              type="button"
              className="flex-1 inline-flex items-center justify-center rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-sm h-10 px-4 py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              onClick={() => handleSave(true)}
              disabled={saving || !name || !price}
            >
              {saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Eye className="mr-2 h-4 w-4" />}
              {saving ? (isEditing ? 'Mise à jour...' : 'Publication...') : (isEditing ? 'Mettre à jour' : 'Publier')}
            </button>
            <button
              type="button"
              className="flex-1 inline-flex items-center justify-center rounded-lg border border-emerald-200 text-emerald-700 hover:bg-emerald-50 font-medium text-sm h-10 px-4 py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            ...
            <button
              type="button"
              className="shrink-0 inline-flex items-center justify-center rounded-lg text-sm font-medium h-10 px-4 py-2 hover:bg-muted transition-colors cursor-pointer"
